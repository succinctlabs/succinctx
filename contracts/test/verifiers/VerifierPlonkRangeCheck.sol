...
contract PlonkVerifier {
    ...
    uint256 private constant g2_srs_1_x_0 = 4063571233546661467734799708300131386289831282909879383778035852630874856695;
    uint256 private constant g2_srs_1_x_1 = 4647926734714541979239671019576593881493945862077482063229589788724791450632;
    uint256 private constant g2_srs_1_y_0 =
        15514169863847815805861354127833333882511661051872364096032336682739266504631;
    uint256 private constant g2_srs_1_y_1 =
        14981245089384445918999453598516659596327553649108599315427712934518517584091;

    // ----------------------- vk ---------------------
    uint256 private constant vk_domain_size = 64;
    uint256 private constant vk_inv_domain_size =
        21546239076966786546898805655487630165289796206659533807077919746160561487873;
    uint256 private constant vk_omega = 9088801421649573101014283686030284801466796108869023335878462724291607593530;
    uint256 private constant vk_ql_com_x = 8241117109803098541098613984488731009721160645379011473335652343529535266960;
    uint256 private constant vk_ql_com_y = 20722965259158873821120253635967463269877724605143204728344162493690267204338;
    uint256 private constant vk_qr_com_x = 6655659571137710427173880313823537029253828787683560125550266937179191868532;
    uint256 private constant vk_qr_com_y = 13808918852189657493040000991883636660567905880489122692410877765981247442335;
    uint256 private constant vk_qm_com_x = 9617414438893381633510036004229347488936383320496724899204335238610971458841;
    uint256 private constant vk_qm_com_y = 9139778173198521885297402021941671824119034286476198203166267405240154924689;
    uint256 private constant vk_qo_com_x = 6753944061897297345277777124783567586429547954466671056344964801972446730543;
    uint256 private constant vk_qo_com_y = 18520936095139799918057373004179121242639392163222661288456393604460483299493;
    uint256 private constant vk_qk_com_x = 15533273441448245872993712610675610998153353458964021815595706151938181224464;
    uint256 private constant vk_qk_com_y = 7989411376990188833711571569796406054937132408812417153401882189030984814970;
    ...
    uint256 private constant vk_s3_com_x = 21721985565414851830858467827731205202151277206551566969624040213120314605650;
    uint256 private constant vk_s3_com_y = 5607754375458556197432930148286768768082695251348209961717955451483607620014;

    uint256 private constant vk_coset_shift = 5;

    uint256 private constant vk_selector_commitments_commit_api_0_x =
        11452134746239496272478105026865109049072990374204527231978960641100599564380;
    uint256 private constant vk_selector_commitments_commit_api_0_y =
        12432732878880068425747106022374326794072801183995506113323301554838024086459;

    uint256 private constant vk_index_commit_api_0 = 14;
    ...
    function Verify(bytes calldata proof, uint256[] calldata public_inputs) public view returns (bool success) {
        assembly {
            ...
            function hash_fr(x, y, mPtr) -> res {
                ...
                for { let i := 0 } lt(i, 0x10) { i := add(i, 1) } {
                    // mPtr <- [xx, xx, ..,  | 0, 0, .. 0  ||    b2   ]
                    mstore8(offset, 0x00)
            ...
            function compute_gamma_kzg(aproof) {
                ...
                let check_staticcall :=
                    staticcall(gas(), 0x2, add(mPtr, start_input), size_input, add(state, state_gamma_kzg), 0x20)
                if eq(check_staticcall, 0) { error_verify() }
            ...
            function compute_commitment_linearised_polynomial_ec(aproof, s1, s2) {
                ...
                let commits_api :=
                    add(aproof, add(proof_openings_selector_commit_api_at_zeta, mul(vk_nb_commitments_commit_api, 0x20)))
                for { let i := 0 } lt(i, vk_nb_commitments_commit_api) { i := add(i, 1) } {
