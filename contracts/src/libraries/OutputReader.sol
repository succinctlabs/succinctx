...
library OutputReader {
    ...
    function readUint128(bytes memory _output, uint256 _offset) internal pure returns (uint256, uint128) {
        uint128 value;
        assembly {
    ...
    function readUint32(bytes memory _output, uint256 _offset) internal pure returns (uint256, uint32) {
        uint32 value;
        assembly {
