...
contract PlonkVerifier {
    ...
    uint256 private constant g2_srs_1_x_0 =
        8092372495400684101491705897324646255244873767278146792539818137343983415338;
    uint256 private constant g2_srs_1_x_1 =
        17267001818546224117620126850851175661030766706904745275133569110744678569705;
    uint256 private constant g2_srs_1_y_0 =
        18322000439618828872938506585690152355173339417118410557175261086259665139770;
    uint256 private constant g2_srs_1_y_1 =
        18361807460194502705595998955236116550325284436398091999656305930880342338897;
    ...
    uint256 private constant vk_ql_com_x =
        10475058859656454050695647591133172935508386459718059756898410999806106551072;
    uint256 private constant vk_ql_com_y =
        6213293053341394200050131794243928751814286941843296559783203899744980812514;
    uint256 private constant vk_qr_com_x =
        7945139303378761555058466625722713607707726441206159286869501528418026434149;
    uint256 private constant vk_qr_com_y =
        156590339282025629533899133000896162239385491129921159716602619014566566552;
    uint256 private constant vk_qm_com_x =
        13992154963319852095604792769360675745515593362505622350879424943943884278192;
    uint256 private constant vk_qm_com_y =
        20766012900377030563947776646862657527189870601611259870484355975445050379609;
    uint256 private constant vk_qo_com_x =
        8470301768743093139793165421641707313403445331488881272110105479663725995698;
    uint256 private constant vk_qo_com_y =
        21592895551100565151417052280131103082580614486310904268527641906481602947;
    uint256 private constant vk_qk_com_x =
        5654499058770597633228728857640171762263824376475793805538552412908288004447;
    uint256 private constant vk_qk_com_y =
        6419423782223253913146410978605915715045763868982098455567651000305029892860;

    uint256 private constant vk_s1_com_x =
        3784233983189669014310841948904857803160351135759376541936942216013730316026;
    uint256 private constant vk_s1_com_y =
        4129762303298768564961482720784423245788144573686711108417866217535238720827;

    uint256 private constant vk_s2_com_x =
        14325725790615933789083728643797722641545097765592717304379756769414822396384;
    uint256 private constant vk_s2_com_y =
        9206760818634532501699551379878233042314021497306309900329293412337859918890;

    uint256 private constant vk_s3_com_x =
        1889439673607470886202316687133798310780298929819093754909820878439514588802;
    uint256 private constant vk_s3_com_y =
        19385229208858775427536138610836306884212006605102248507339743025747156971303;

    uint256 private constant vk_coset_shift = 5;

    uint256 private constant vk_selector_commitments_commit_api_0_x =
        4117073030668173863289093683350495260595200061919445680132579228612965095242;
    ...
    uint256 private constant vk_index_commit_api_0 = 12269855;
        ...
        assembly {
            ...
            function check_proof_size(actual_proof_size) {
                let expected_proof_size := add(
                    0x340,
                    mul(vk_nb_commitments_commit_api, 0x60)
                )
            ...
            function derive_gamma(aproof, nb_pi, pi) -> gamma_not_reduced {
                ...
                let size_wire_commitments_commit_api_in_bytes := mul(
                    vk_nb_commitments_commit_api,
                    0x40
                )
                calldatacopy(
                    _mPtr,
                    _proof,
                    size_wire_commitments_commit_api_in_bytes
                )
            ...
            function compute_gamma_kzg(aproof) {
                ...
                mstore(
                    add(mPtr, offset),
                    vk_selector_commitments_commit_api_0_x
                )
                mstore(
                    add(mPtr, add(offset, 0x20)),
                    vk_selector_commitments_commit_api_0_y
                )
                ...
                let size_input := add(
                    0x16,
                    mul(vk_nb_commitments_commit_api, 3)
                ) // number of 32bytes elmts = 0x16 (zeta+2*7+7 for the digests+openings) + 2*vk_nb_commitments_commit_api (for the commitments of the selectors) + vk_nb_commitments_commit_api (for the openings of the selectors)
