...
pub struct SigVerificationAirParameters<F: Field, R: CubicParameters<F>>(pub PhantomData<(F, R)>);

impl<F: PrimeField64, R: CubicParameters<F>> AirParameters for SigVerificationAirParameters<F, R> {
    type Field = F;
    type CubicParams = R;
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(bound = "")]
pub struct SigVerificationAir<F: PrimeField64, R: CubicParameters<F>> {
    pub air: Chip<SigVerificationAirParameters<F, R>>,
    pub trace_data: AirTraceData<SigVerificationAirParameters<F, R>>,
...
impl<F: PrimeField64, R: CubicParameters<F>> SigVerificationAir<F, R> {
    /// Creates a new instance of the SigVerificationAir
    pub fn new() -> Self {
        let mut builder = AirBuilder::<SigVerificationAirParameters<F, R>>::new();
