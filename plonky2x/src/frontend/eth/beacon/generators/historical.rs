...
const HISTORICAL_SUMMARY_DEPTH: usize = 33;
const BLOCK_ROOT_DEPTH: usize = 14;
...
pub struct BeaconHistoricalBlockGenerator<F: RichField + Extendable<D>, const D: usize> {
    ...
    pub historical_summary_root: Bytes32Variable,
    pub historical_summary_proof: Vec<Bytes32Variable>,
    pub block_root_proof: Vec<Bytes32Variable>,
...
impl<F: RichField + Extendable<D>, const D: usize> BeaconHistoricalBlockGenerator<F, D> {
    pub fn new<L: PlonkParameters<D>>(
        ...
        Self {
            ...
            historical_summary_root: builder.init::<Bytes32Variable>(),
            historical_summary_proof: (0..HISTORICAL_SUMMARY_DEPTH)
                .map(|_| builder.init::<Bytes32Variable>())
                .collect::<Vec<_>>(),
            block_root_proof: (0..BLOCK_ROOT_DEPTH)
                .map(|_| builder.init::<Bytes32Variable>())
                .collect::<Vec<_>>(),
    ...
    for BeaconHistoricalBlockGenerator<F, D>
{
    ...
    fn run_once(&self, witness: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
        ...
        self.historical_summary_root
            .set(out_buffer, bytes32!(result.historical_summary_root));
        for i in 0..HISTORICAL_SUMMARY_DEPTH {
            self.historical_summary_proof[i]
                .set(out_buffer, bytes32!(result.historical_summary_proof[i]));
        }
        for i in 0..BLOCK_ROOT_DEPTH {
            self.block_root_proof[i].set(out_buffer, bytes32!(result.block_root_proof[i]));
        }
    }
    ...
    fn serialize(&self, dst: &mut Vec<u8>, common_data: &CommonCircuitData<F, D>) -> IoResult<()> {
        ...
        dst.write_target_vec(&self.historical_summary_root.targets())?;
        for i in 0..HISTORICAL_SUMMARY_DEPTH {
            dst.write_target_vec(&self.historical_summary_proof[i].targets())?;
        }
        for i in 0..BLOCK_ROOT_DEPTH {
            dst.write_target_vec(&self.block_root_proof[i].targets())?;
        }
        Ok(())
    }
    ...
    fn deserialize(src: &mut Buffer, common_data: &CommonCircuitData<F, D>) -> IoResult<Self> {
        ...
        let historical_summary_root = Bytes32Variable::from_targets(&src.read_target_vec()?);
        let mut historical_summary_proof = Vec::new();
        for i in 0..HISTORICAL_SUMMARY_DEPTH {
            historical_summary_proof.push(Bytes32Variable::from_targets(&src.read_target_vec()?));
        }
        let mut block_root_proof = Vec::new();
        for i in 0..BLOCK_ROOT_DEPTH {
            block_root_proof.push(Bytes32Variable::from_targets(&src.read_target_vec()?));
        }
