...
const SIGNING_KEYS_MAPPING_NAME: &str = "0xeb2b7ad4d8ce5610cfb46470f03b14c197c2b751077c70209c5d0139f7c79ee9";
...
impl LidoUtils {
    pub fn new(rpc_url: &str) -> LidoUtils {
        ...
        let contract = NodeOperatorRegistry::new(NODE_OPERATOR_REGISTRY_ADDR.parse::<Address>().unwrap(), client);
        LidoUtils { provider: provider1, contract }
    }

    pub async fn get_node_operator_info(&self, block: BlockId, operator_id: u64) -> Result<EIP1186ProofResponse> {
        ...
        let storage_value = self.provider
            ...
            .await?;
        println!("Storage at {} in block {:?}: {:?}", NODE_OPERATOR_REGISTRY_ADDR.parse::<Address>()?, block, storage_value);
        ...
        let proof = self.provider.get_proof(
            NODE_OPERATOR_REGISTRY_ADDR.parse::<Address>()?, vec![location], Some(block.into())
        ).await?;
        Ok(proof)
    }
    ...
    pub async fn get_operator_key_info(&self, block: BlockId, operator_id: u64, key_idx: u64) -> Result<EIP1186ProofResponse> {
        ...
        let storage_1 = self.provider
            .get_storage_at(NODE_OPERATOR_REGISTRY_ADDR.parse::<Address>()?, key_offset, Some(block.into()))
            .await?;
        println!("Storage at {} in block {}: {:?}", NODE_OPERATOR_REGISTRY_ADDR.parse::<Address>()?, key_offset, storage_1);

        let storage_2 = self.provider
            .get_storage_at(NODE_OPERATOR_REGISTRY_ADDR.parse::<Address>()?, key_offset_plus_1_h256, Some(block.into()))
            .await?;
        println!("Storage at {} in block {}: {:?}", NODE_OPERATOR_REGISTRY_ADDR.parse::<Address>()?, key_offset_plus_1_h256, storage_2);

        // TODO verify that signing_key.0 == truncate(concat(storage_1, storage_2), 48)

        let proof = self.provider.get_proof(
            NODE_OPERATOR_REGISTRY_ADDR.parse::<Address>()?, vec![key_offset, key_offset_plus_1_h256], Some(block.into())
        ).await?;
        Ok(proof)
    }
}
