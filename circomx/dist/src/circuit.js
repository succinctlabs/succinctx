...
const NODE_OPTIONS = "--huge-max-old-generation-size --max-old-space-size=2048000 --initial-old-space-size=2048000 --no-global-gc-scheduling --no-incremental-marking --max-semi-space-size=2048000 --initial-heap-size=2048000 --expose-gc";
// @ts-ignore
...
export class Circuit {
    constructor() { }
    build(snarkjsPath, circomPath, ptauPath) {
        const circuit = this.circuitName();
        // Create build dir if not exists
        if (!fs.existsSync("build")) {
            fs.mkdirSync("build");
        }
        executeCommand(`${circomPath} circuits/${circuit}.circom --O1 --r1cs --sym --c --output build`);
        const circuitName = circuit === "main" ? "main_c" : circuit;
        const buildDirName = `${circuitName}_cpp`;
        executeCommand(`make -C build/${buildDirName}/`);
        executeCommand(`cp build/${buildDirName}/${circuitName} build/${circuitName}`);
        executeCommand(`cp build/${buildDirName}/${circuitName}.dat build/${circuitName}.dat`);
        executeCommand(`rm -rf build/${buildDirName}`);
        executeCommand(`rm -rf build/${circuit}_cpp`);
        executeCommand(`node ${NODE_OPTIONS} ${snarkjsPath} zkey new build/${circuitName}.r1cs ${ptauPath} build/p1.zkey`);
        executeCommand(`node ${NODE_OPTIONS} ${snarkjsPath} zkey export verificationkey build/p1.zkey build/vkey.json`);
        executeCommand(`node ${NODE_OPTIONS} ${snarkjsPath} zkey export solidityverifier build/p1.zkey build/FunctionVerifier.sol`);
        // Replace first line of FunctionVerifier.sol with "pragma solidity ^0.8.0;"
        let solidityVerifier = fs.readFileSync("build/FunctionVerifier.sol", "utf8");
        ...
        solidityVerifier = solidityVerifier.replaceAll("_pB, _pC", 
        // for some reason, uint256[2][2] memory _pB has two words (two lengths?) prepended to it
        // and calldata doesn't have it
        "add(_pB, 64), _pC");
        solidityVerifier = solidityVerifier.replaceAll("pragma solidity >=0.7.0 <0.9.0;", "pragma solidity ^0.8.16;");
        solidityVerifier += `
...
`;
    ...
    async prove(rapidsnarkPath, inputJsonPath) {
        const circuit = this.circuitName();
        const circuitName = circuit === "main" ? "main_c" : circuit;
        ...
        executeCommand(`${rapidsnarkPath} build/p1.zkey witness.wtns proof.json public.json`);
    ...
    entrypoint() {
        ...
        yargs(hideBin(process.argv))
            .command("build", "Run build commands", (yargs) => {
            yargs
                .option("snarkjs", {
            ...
            const snarkjsPath = args.snarkjs;
            const circomPath = args.circom;
            const ptauPath = args.ptau;
            this.build(snarkjsPath, circomPath, ptauPath);
        })
            .command("prove", "Run prove commands", (yargs) => {
            yargs
                .option("input-json", {
            ...
            const rapidsnarkPath = args.rapidsnark;
            const inputJsonPath = args["input-json"];
